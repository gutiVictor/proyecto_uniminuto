"""
Dashboard de Análisis de Despachos NEXO Aura
...
try:
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                f"logs/nexo_dashboard_{datetime.now().strftime('%Y%m%d')}.log",
                encoding='utf-8'
            ),
...
COLOR_CUMPLE = '#10B981'      # Esmeralda: pedido cumplido dentro de SLA
COLOR_NO_CUMPLE = '#F43F5E'   # Rosa/Rojo: pedido fuera de SLA
COLOR_PTE = '#F59E0B'         # Ámbar: pedido pendiente (PTE)
COLOR_PRIMARY = '#38BDF8'     # Azul cielo: color principal de la marca
...
# ──────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="NEXO – Análisis de Despachos",  # Título en pestaña del navegador
...
<style>
    ...
    [data-testid="stSidebar"] h3 { color: #38BDF8; }
    ...
    .rec-success { border-left-color: #10B981; }  /* Verde: éxito */
    .rec-warning { border-left-color: #F59E0B; }  /* Ámbar: advertencia */
    .rec-error   { border-left-color: #F43F5E; }  /* Rojo: error */
    .rec-info    { border-left-color: #38BDF8; }  /* Azul: información */

    /* Encabezados de tablas de datos */
    .dataframe thead th { background: #1E293B !important; color: #F8FAFC !important; }
</style>
...
def sidebar_filtros(df_procesado: pd.DataFrame) -> tuple:
    ...
    """
    st.sidebar.markdown("## 📦 NEXO Despachos")
    st.sidebar.markdown("---")
...
def mostrar_graficos(processor, df_filtrado: pd.DataFrame, debug_mode: bool = False) -> None:
    ...
    if analisis_c is not None and len(analisis_c) > 0:
        ...
        fig3 = px.bar(
            ...
            color_continuous_scale=['#F43F5E', '#F59E0B', '#10B981'],  # Rojo→Ámbar→Verde
...
def main():
    ...
    uploaded = st.sidebar.file_uploader(
        ...
        type=['xlsx', 'xls'],
        help="Sube el archivo de Seguimiento de Despachos NEXO con las columnas esperadas"
    )

    # ── PANTALLA DE BIENVENIDA (si no hay archivo cargado) ──
    if uploaded is None:
        st.markdown("# 📦 NEXO – Análisis de Despachos")
        st.markdown("---")
    ...
    try:
        ...
        if ind_filtrado:
            ...
            st.sidebar.download_button(
                btn_label,
                data=mega_buf,
                file_name=f"Reporte_NEXO_Analisis_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    ...
    st.markdown("# 📊 Dashboard de Despachos NEXO Aura `v2.0` 🚀")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Aplicación NEXO Dashboard iniciada")
    main()